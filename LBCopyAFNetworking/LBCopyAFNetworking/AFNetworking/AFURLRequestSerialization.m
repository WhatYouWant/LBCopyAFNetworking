...
@property (readwrite, nonatomic, strong) dispatch_queue_t requestHeaderMofificationQueue;
...
- (instancetype)init {
    ...
    self.requestHeaderMofificationQueue = dispatch_queue_create("requestHeaderModificationQueue", DISPATCH_QUEUE_CONCURRENT);
...
- (NSDictionary *)HTTPRequestHeaders {
    ...
    dispatch_sync(self.requestHeaderMofificationQueue, ^{
...
- (void)setValue:(NSString *)value forHTTPHeaderField:(NSString *)field {
    dispatch_barrier_async(self.requestHeaderMofificationQueue, ^{
                                     ...
                                     error:(NSError *__autoreleasing  _Nullable *)error
{
    ...
    for (NSString *keyPath in AFHTTPRequestSerializerObservedKeyPaths()) {
        if ([self.mutableObservedChangedKeyPaths containsObject:keyPath]) {
            [mutableRequest setValue:[self valueForKey:keyPath] forHTTPHeaderField:keyPath];
        }
    }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    self.mutableHTTPRequestHeaders = [[aDecoder decodeObjectOfClass:[NSDictionary class] forKey:NSStringFromSelector(@selector(mutableHTTPRequestHeaders))] mutableCopy];
    self.queryStringSerializationStyle = (AFHTTPRequestQueryStringSerializationStyle)[[aDecoder decodeObjectOfClass:[NSNumber class] forKey:NSStringFromSelector(@selector(queryStringSerializationStyle))] unsignedIntegerValue];
    return self;
}
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    dispatch_sync(self.requestHeaderMofificationQueue, ^{
        serializer.mutableHTTPRequestHeaders = [serializer.mutableHTTPRequestHeaders mutableCopyWithZone:zone];
    });
    serializer.queryStringSerializationStyle = self.queryStringSerializationStyle;
    serializer.queryStringSerialization = serializer.queryStringSerialization;
...
#if (defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000) || (defined(__MAC_OS_X_VERSION_MAX_ALLOWED) && __MAX_OS_X_VERSION_MAX_ALLOWED >= 1100)
@synthesize delegate;
#endif
...
- (NSInteger)read:(uint8_t *)buffer
    ...
    if (_phase == AFEncapsulationBoundaryPhase) {
        NSData *encapsulationBoundaryData = [([self hasInitialBoundary] ? AFMultipartFormFinalBoundary(self.boundary) : AFMultipartFormEncapsulationBoundary(self.boundary)) dataUsingEncoding:self.stringEncoding];
...
- (NSURLRequest *)requestBySerializingRequest:(NSURLRequest *)request
    ...
    [self.HTTPRequestHeaders enumerateKeysAndObjectsUsingBlock:^(id field, id value, BOOL * __unused stop) {
        if (![request valueForHTTPHeaderField:field]) { //为什么要取反
            [mutableRequest setValue:value forHTTPHeaderField:field];
        }
    }];
